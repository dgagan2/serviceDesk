...
export const createServiceItem = async (req, res) => {
  const { nameItem, idCategory, itemImage, itemDescription } = req.body
  if (!nameItem || !idCategory || !itemDescription || !itemImage) return res.status(400).json({ message: 'Debe ingresar todos los datos' })
  try {
    ...
    const newItem = await addNewService(nameItem, idCategory, itemImage, itemDescription)
...
const addNewService = async (nameItem, idCategory, itemImage, itemDescription) => {
  try {
